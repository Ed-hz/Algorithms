...
typedef struct BTNode
{
    ...
    //?  0号元素用来指向自己的双亲 key,recptr未用
} BTNode, *BTree;

typedef struct
{
    BTNode *pt; //*  指向找到的结点
    int i;      //*  找到的结点 在结点中的关键字序号
...
//- 若查找成功tag=1，指针pt所指结点中第i个关键字等于K
//- 否则特征值tag=0，等于K的关键字 插入在指针Pt结点中第i和i+1个关键字间
Result SearchBTree(BTree T, int K)
{
    BTree p = T, q = NULL; //* p指向待查结点，q指向p的双亲
    Status found = FALSE;
    int i = 0;
    Result r; //- 初始化
    ...
    while (p && !found)
    {
        ...
        {
            q = p;
            p = p->node[i].ptr; //- 在子结点继续寻找
        }
    } //? 找完至少有 p->node[i].key≤K<p->node[i+1].key
...
//$插入
//@ 单结点插入
//& 将key、r和ap分别插入到 q->key[i+1]、recptr[i+1]和ptr[i+1]中
void Insert(BTree *q, int i, int key, BTree ap)
{
    int j;
    for (j = (*q)->keynum; j > i; j--) //* 整体右移 空出(*q)->node[i+1]
        (*q)->node[j + 1] = (*q)->node[j];
...
//@插入过程
//- 在m阶B树T上结点*q的key[i]与key[i+1]之间 插入关键字K的指针r
...
void InsertBTree(BTree *T, int key, BTree q, int i)
{
    ...
    while (q && !finished)
    {
        ...
        else                 //- 分裂结点*q
        {
            ...
            rx = q->node[s].recptr;
            split(&q, &ap);
            //- 将q->key,q->ptr和q->recptr[s+1..m]移入新结点*ap
    ...
    if (!finished)
        //-  T空(参数q初值为NULL)或 根结点已分裂为结点*q和*ap
        NewRoot(T, rx, ap);
