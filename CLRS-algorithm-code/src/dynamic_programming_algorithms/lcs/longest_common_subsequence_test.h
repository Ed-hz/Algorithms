...
#include <string>
using IntroductionToAlgorithm::DynamicProgrammingAlgorithm::
    longest_common_subsequence;
//! longest_common_subsequence_test：测试最长公共子数组
/*!
 *
 * 三组数据：
 * s1为空字符串s2为非空字符串、s1和s2为常规字符串且二者内容不等、s1和s2为常规字符串且二者内容相等
 */
TEST(test_longest_common_subsequence, test1) {

  {
    ...
    auto length = longest_common_subsequence(s1.begin(), s1.end(), s2.begin(),
                                             s2.end(), result.begin());
