...
/*!
 *
 * `BinaryTreeNodeTest`是 `::testing::Test`
...
TEST_F(BinaryTreeNodeTest, test_default_node) {
  ...
  EXPECT_FALSE(default_node->parent
                   .lock()); // weak_ptr<T> 必须通过lock()获取一个shared_ptr<T>
}
//! binary_tree_node_test：测试二叉树节点
/*!
 *
...
TEST_F(BinaryTreeNodeTest, test_to_string) {
  EXPECT_EQ(default_node->to_string(),
            "node:0\n\t parent:null_ptr\n\t left_child:nullptr\n\t "
            "right_child:nullptr");
  EXPECT_EQ(root_node->to_string(),
            "node:0\n\t parent:null_ptr\n\t left_child:1\n\t right_child:2");
}
//! binary_tree_node_test：测试二叉树节点
/*!
