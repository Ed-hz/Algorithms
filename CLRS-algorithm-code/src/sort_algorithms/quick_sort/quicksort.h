 ...
 * \param partition_iter:
 * 指定划分元素的对应的迭代器（也可以是指向数组中某元素的指针） \param compare:
 ...
 *   - 循环：维持循环不变式：
 * A[p...smaller_next-1]始终小于A[r]，A[smaller_next...current-1]始终大于A[r]。开始对A[current]进行判别：
 *          - 若A[current]<A[r] 则交换 A[current]与 A[smaller_next],
 * current右移（进行下一个元素的判断），smaller_next右移（维持不变式）
...
Iterator partition(const Iterator begin, const Iterator end,
  ...
  assert(size >= 0);
  if (size == 0)
    return end; //空序列，返回end
                //            if(size<=1)
                //                return begin;
  assert(std::distance(begin, partition_iter) >= 0 &&
         std::distance(partition_iter, end) > 0);
  auto smaller_next =
      begin; //指向比key小的元素区间的下一个(即大于等于key元素区间的第一个），其中key为序列最后一个元素
 ...
 * \param compare:
 * 一个可调用对象，可用于比较两个对象的小于比较，默认为std::less<T> \return void
 *
 ...
 * - 最坏时间复杂度 O(n^2)， 期望时间复杂度
 * O(nlgn)。它平均性能非常好，是实际排序应用中最好的选择
 * - 原地排序
 */
template <typename Iterator,
          typename CompareType =
...
void quick_sort(const Iterator begin, const Iterator end,
  ...
  auto partitioned_iter =
      partition(begin, end, end - 1,
                compare); // 以最后一个元素作为划分元素，返回其顺序统计量
