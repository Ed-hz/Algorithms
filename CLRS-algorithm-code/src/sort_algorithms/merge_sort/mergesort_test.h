 ...
 *
 * 五组数组：
 * 左右对半、左边只有1个元素、右边只有一个1元素、左边只有0个元素、右边只有一个0元素
 * 参考函数： std::sort()函数
 */
TEST(test_merge, test_C_array) {
  ...
  EXPECT_EQ(std::vector<int>(data3, data3 + 10),
            std::vector<int>(comparedata3, comparedata3 + 10));
  EXPECT_EQ(std::vector<int>(data4, data4 + 10),
            std::vector<int>(comparedata4, comparedata4 + 10));
  EXPECT_EQ(std::vector<int>(data5, data5 + 10),
            std::vector<int>(comparedata5, comparedata5 + 10));
}
