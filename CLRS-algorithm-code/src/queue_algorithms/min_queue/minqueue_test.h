...
/*!
 *
 * `MinQueueTest`是 `::testing::Test`
 * 的子类。它主要用于为每一个`TEST_F`准备测试环境
 */
class MinQueueTest : public ::testing::Test {
public:
  typedef std::function<bool(std::shared_ptr<int>, std::shared_ptr<int>)>
      Int_Compare_Type; /*!<
                           当最小优先级队列存放的是int*数据时，相应的比较函数对象*/
  typedef std::function<int &(std::shared_ptr<int>)>
      Int_Get_Type; /*!< 当最小优先级队列存放的是int*数据时，获取key的函数对象*/
  typedef std::function<bool(std::shared_ptr<Node>, std::shared_ptr<Node>)>
      Struct_Compare_Type; /*!<
                              当最小优先级队列存放的是Node*数据时，相应的比较函数对象*/
  typedef std::function<double &(std::shared_ptr<Node>)>
      Struct_Get_Type;                                 /*!<
                                                          当最小优先级队列存放的是Node*数据时，获取key的函数对象*/
  ...
  typedef MinQueue<int, int> Int_MinQueue_Type;        /*!<
                                                          存放的是Node*数据的最小优先级队列，Node的关键字是double类型
                                                        */
protected:
  void SetUp() {
    Struct_Compare_Type node_compare = [](std::shared_ptr<Node> first,
                                          std::shared_ptr<Node> second) {
      return first->key < second->key;
    };
    Struct_Get_Type node_get = [](std::shared_ptr<Node> node) -> double & {
      return node->key;
    };
    _strcut_minqueue =
        std::make_shared<Struct_MinQueue_Type>(node_compare, node_get);

    Int_Compare_Type int_compare = [](std::shared_ptr<int> first,
                                      std::shared_ptr<int> second) {
      return *first < *second;
    };
    Int_Get_Type int_get = [](std::shared_ptr<int> node) -> int & {
      return *node;
    };
    _int_minqueue = std::make_shared<Int_MinQueue_Type>(int_compare, int_get);
  }
  void TearDown() {}
  std::shared_ptr<Struct_MinQueue_Type>
      _strcut_minqueue; /*!< 指向一个最小优先级队列*/
  std::shared_ptr<Int_MinQueue_Type>
      _int_minqueue; /*!< 指向一个最小优先级队列*/
};
...
TEST_F(MinQueueTest, test_index_inqueue) {
  ...
  EXPECT_THROW(_strcut_minqueue->index_inqueue(std::shared_ptr<Node>()),
               std::invalid_argument);
  EXPECT_THROW(_int_minqueue->index_inqueue(std::shared_ptr<int>()),
               std::invalid_argument);
  for (int i = 1; i < Q_NUM; i++) {
