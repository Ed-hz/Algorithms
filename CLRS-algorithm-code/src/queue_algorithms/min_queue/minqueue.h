 ...
 *   - _compare:是一个std::function<bool
 * (std::shared_ptr<T>,std::shared_ptr<T>)>类型的对象，它用于队列中两个数据的大小比较。
 *
 */
template <typename T, typename TKeyType> class MinQueue {
public:
  typedef std::function<bool(std::shared_ptr<T>, std::shared_ptr<T>)>
      CompareType; /*!<
                      一个可调用对象类型，该类型的对象可用于比较两个std::shared_ptr<T>的小于比较*/
  typedef std::function<TKeyType &(std::shared_ptr<T>)>
      GetKeyType; /*!<
                     一个可调用对象，该类型的对象可用于获取std::shared_ptr<T>的关键字，并修改该关键字（返回的是关键字的引用）*/
  //!显式构造函数
  /*!
   *  \param
   * compare:一个可调用对象，可用于比较两个std::shared_ptr<T>对象的小于比较
   *  \param
   * getKey：一个可调用对象，它的参数为std::shared_ptr<T>，返回TKey&。通过它的返回值可以获取T的key，也可以修改T的key
   *
   */
  MinQueue(CompareType compare, GetKeyType getKey)
      : _size(0), _compare(compare), _getKey(getKey) {}
  //!显式构造函数
  /*!
   ...
   *
   */
  MinQueue(std::size_t reseve_size, CompareType compare, GetKeyType getKey)
      : _size(0), _compare(compare), _getKey(getKey) {
  ...
  int index_inqueue(std::shared_ptr<T> element) {
    if (!element)
      throw std::invalid_argument(
          "is_inqueue error: element must not be nullptr!");
  ...
  void decreate_key(std::size_t element_index, TKeyType new_key) {
    if (element_index >= _size)
      throw std::invalid_argument(
          "decreate_key error: element_index must less than _size!");
    if (new_key > _getKey(_data.at(element_index)))
      throw std::invalid_argument(
          "decreate_key error: new_key less _data.at(element_index)->key!");
