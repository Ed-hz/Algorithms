  ...
  for (int a = 1; a < 4; a++) {
    for (int k = 0; k < P.size(); k++)
      for (int q = 0; q < P.size(); q++)
        if (k == 0)
          EXPECT_TRUE(is_end_with(P.begin(), P.begin() + k, P.begin() + q, a))
              << "a:" << a << "\t k:" << k
              << "\t q:" << q; // Pk是否是( Pq a)的后缀
        else if (a == 2 && (k <= q + 1))
          EXPECT_TRUE(is_end_with(P.begin(), P.begin() + k, P.begin() + q, a))
          ...
          EXPECT_FALSE(is_end_with(P.begin(), P.begin() + k, P.begin() + q, a))
              << "a:" << a << "\t k:" << k
              << "\t q:" << q; // Pk是否是( Pq a)的后缀
  }
}
 ...
 *
 * 三组数据：
 * 匹配字符串T为空字符串、匹配字符串T为非空字符串（同一个字符）、匹配字符串T为非空字符串（不同字符）
 */
TEST(MatchTest, test_finite_automaton_match) {
  ...
  { //******  测试T为空
    ...
    EXPECT_THROW(finite_automaton_match(T.begin(), T.end(), P.begin(), P.end(),
                                        M.begin(), M.end()),
  ...
  { //******  测试T为非空
    ...
    std::vector<int> M = {90};
    EXPECT_EQ(finite_automaton_match(T.begin(), T.end(), P.begin(), P.end(),
                                     M.begin(), M.end()),
              std::vector<int>({0, 1, 2, 3, 4}));
  }
  { //******  测试T为非空  《算法导论图32-1》
    std::vector<char> T = {'a', 'b', 'c', 'a', 'b', 'a', 'a',
                           'b', 'c', 'a', 'b', 'a', 'c'};
