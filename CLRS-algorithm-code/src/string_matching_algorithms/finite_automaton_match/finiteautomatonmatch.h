  ...
  while (iter < endM) //继续查找，有重复字符则抛出异常
  {
    if (*iter == a)
      throw std::invalid_argument(
          std::string("index_of_M error: M has dulplicate character ") +
          os.str() + "!");
    iter++;
  }
  if (result_iter == endM)
    throw std::invalid_argument(
        std::string("index_of_M error: M has no charactor ") + os.str() + "!");
 ...
 */
template <typename Iterator>
bool is_end_with(Iterator begin, Iterator k_iter, Iterator q_iter,
                 typename std::iterator_traits<Iterator>::value_type a) {
  auto k = std::distance(begin, k_iter);
  auto q = std::distance(begin, q_iter);
  if (k < 0 || q < 0)
    throw std::invalid_argument(
        "is_end_with error:must be k_iter>=begin and q_iter>=begin!");
 ...
 *   - 对每个字符a属于有限字母集合a，寻找Pk是 (Pq a) 后缀的最大的k，则
 * delt(q,a)=k
 *
...
void get_delta(const PIterator P_begin, const PIterator P_end,
  ...
  static_assert(std::is_same<T1, T2>::value,
                "get_delt error:mode sequence P and sequence M must contain "
                "same type value!");
  ...
  if (p_len < 0)
    throw std::invalid_argument(
        "finite_automaton_match error:P_begin must <= P_end");
  if (m_len <= 0)
    throw std::invalid_argument(
        "finite_automaton_match error:M_begin must < M_end");

  delta.clear(); //矩阵清零

  for (long long q = 0; q <= p_len; q++) {
 ...
 *
 * 有限自动机开始于状态
 * q_0，每次读入输入字符串的一个字符。如果有限自动机在状态q时读入了字符a，则它从状态q变成了状态
 * delt(q,a)，
 * 进行了一次转移。每当当前状态q属于A时，则说明状态机接受了迄今为止所读入的字符串。
 *
 * 有限自动机引入了一个函数 phai,称为终态函数。定义 M*
 * 表示所有的有限长度的字符串的集合，该字符串是由所有字母表M中的字符组成。
 * 长度为0的空字符串记为e,e属于M*. phai 是从 M*到Q的函数，满足：
 * pai(w)是有限自动机扫描字符串w后的终止状态。 其中 w 属于 M*。因此当且仅当
 * phai(w)属于A时，有限自动机接受字符串 w。我们可以用转移函数定义 phai:
 *
 * - phai(e)=q_0
 ...
 * 首先定义一个辅助函数 sigma，称之为P的后缀函数。函数 sigma是一个
 * M*到{0,1,....m}上的映射：
 *
 ...
 * #### 匹配算法
 * - 遍历T，i从1到n:
 *   - 计算 q=delt(q,T[i])。如果 q==m，则偏移 i-m是有效偏移点，将 i-m
 * 加入结果std::vector中
 *
 * >这里要求n>=0，m>=0(模式序列可以为空字符串)；否则抛出异常
 * >
 ...
 * ### 算法性能
 *
 * 有限自动机字符串匹配算法的预处理时间为O(m^3 |M|)，其中|M|
 * 为有限字母集合的大小，匹配时间为O(n)
 *
 */
template <typename IteratorT, typename IteratorP, typename IteratorM>
std::vector<int>
finite_automaton_match(const IteratorT iterT_begin, const IteratorT iterT_end,
  ...
  static_assert(std::is_same<T1, T2>::value,
                "finite_automaton_match error:sequence T and P must contain "
                "same type value!");
  static_assert(std::is_same<T1, T3>::value,
                "finite_automaton_match error:sequence T and M must contain "
                "same type value!");
  ...
  if (lenT < 0)
    throw std::invalid_argument(
        "finite_automaton_match error:iterT_begin must <= iterT_end");
  if (lenP <= 0)
    throw std::invalid_argument(
        "finite_automaton_match error:iterP_begin must < iterP_end");
  if (lenM <= 0)
    throw std::invalid_argument(
        "finite_automaton_match error:iterM_begin must < iterM_end");
  ...
  for (int i = 0; i < lenT; i++) {
    ...
    if (q == lenP) {
      result.push_back(
          i - lenP +
          1); //[0,1,...i] ，其右侧长度为lenP的区间为[i-lenP+1,i-lenP+2,...i]
    }
  }
  return result;
}
