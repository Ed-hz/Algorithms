 ...
 * \param rank:
 * 指定选取的顺序数，0为选取最小的元素，1为选取次小的元素....n表示选取排序为n的元素（从小排序）
 * \param compare:
 * 一个可调用对象，可用于比较两个对象的小于比较，默认为std::less<T> \return
 * 第rank小的元素值
 *
 ...
 *          - 针对这些分组的中位数构成的序列，递归调用
 * good_select，找出中位数的中位数
 *          - 将这个中位数的中位数作为划分主元
 *      - 划分：根据找到的主元对原序列进行划分，假设划分完毕后，主元是第m小
 *      - 判定：
 *          - 若m==k，则找到了这个元素，返回这个主元
 *          - 若m<k ，则说明指定的元素在
 * A[m+1...r]中，且位于这个新数组的第(k-m-1)小，此时递归调用good_select(....)
 *          - 若m>k， 则说明指定的元素在 A[p...m-1]中，且位于这个新数组的第 k
 * 小，此时递归调用good_select(...)
 * - 时间复杂度：最坏情况下时间为O(n)
 * -
...
good_select(const Iterator begin, const Iterator end,
            typename std::iterator_traits<Iterator>::difference_type rank,
  ...
  // ********* 取出这些中值的中值 ************
  T mid_of_middles =
      good_select(middle_nums.begin(), middle_nums.end(),
                  middle_nums.size() / 2, compare); //所有中值的中值
  iter = begin;
  while (std::distance(iter, end) > 0 &&
         *iter != mid_of_middles) //得到中值的中值在原序列中的位置
  {
    iter++;
  }
  //********* 划分 **************
  auto mid_of_middles_iter =
      partition(begin, end, iter, compare); //以中值的中值作为一个划分值
  // ********** 判别 ***************
  auto mid_of_middles_rank =
      std::distance(begin, mid_of_middles_iter); //中值的中值在划分之后的排序
  if (mid_of_middles_rank == rank)               //找到了该排位的数
    return *mid_of_middles_iter;
  else if (mid_of_middles_rank <
           rank) //已知某排位的数位次较低，则指定排位数的元素在它右侧
    return good_select(
        mid_of_middles_iter + 1, end, rank - mid_of_middles_rank - 1,
        compare); // mid_of_middles_iter+1，则找右侧的第rank-mid_of_middles_rank-1位
