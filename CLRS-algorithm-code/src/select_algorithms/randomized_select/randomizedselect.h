...
template <typename IntType> IntType radom_index(IntType begin, IntType end) {
  ...
  static std::random_device
      rd; //每一个std::random_device维持相同的伪随机序列，因此需要用静态变量保证只有一个std::random_device
  std::mt19937 gen(rd());
 ...
 *          - 若m<k ，则说明指定的元素在
 * A[q+1...r]中，且位于这个新数组的第(k-m-1)小，此时递归调用randomized_select(q+1,end,k-m-1)
 *          - 若m>k， 则说明指定的元素在 A[p...q-1]中，且位于这个新数组的第 k
 * 小，此时递归调用randomized_select(begin,q,k)
...
randomized_select(const Iterator begin, const Iterator end,
  ...
  auto partitioned_iter =
      partition(begin, end, begin + radom_index((long long)0, size - 1),
                compare); //随机划分
  ...
  if (distance == rank) //找到了该排位的数
    ...
    return randomized_select(
        partitioned_iter + 1, end, rank - distance - 1,
        compare); //从partitioned_iter+1，则找右侧的第rank-distance-1位
