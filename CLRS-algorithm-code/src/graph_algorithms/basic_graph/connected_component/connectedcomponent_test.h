...
protected:
  void SetUp() {
    ...
    for (int i = C_NUM / 2 + 1; i < C_NUM;
         i++) //边 C_NUM/2--C_NUM/2+1...--C_NUM-1
...
TEST_F(ConnectedComponentTest, test_connected_component) {
  ...
  for (int i = 0; i < C_NUM / 2;
    ...
    EXPECT_EQ(_graph->vertexes.at(i)->node->parent,
              _graph->vertexes.at(1)->node); //所以结点1是最终根结点

  for (int i = C_NUM / 2; i < C_NUM;
       i++) // tree_root:u, tree_root:v,
            // rank小的挂在rank大的之下；若二者rank相等，则u挂在v下
    EXPECT_EQ(_graph->vertexes.at(i)->node->parent,
              _graph->vertexes.at(C_NUM / 2 + 1)
                  ->node); //所以结点`C_NUM/2+1`是最终根结点
}
