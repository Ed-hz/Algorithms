...
void connected_component(std::shared_ptr<GraphType> graph) {
  ...
  if (!graph)
    throw std::invalid_argument(
        "connected_component error: graph must not be nullptr!");
  //*********** 初始化 ****************
  ...
  for (const auto &edge : edges) {
    ...
    if (NodeType::find_set(from_vertex_set_node) !=
        NodeType::find_set(to_vertex_set_node)) {
 ...
 *
 * 在执行 same_component函数之前必须先执行
 * connected_component函数对无向图进行预处理。
 *
 */
template <typename GraphType>
bool same_component(std::shared_ptr<GraphType> graph,
                    typename GraphType::VIDType id1,
                    typename GraphType::VIDType id2) {
  ...
  if (!graph)
    throw std::invalid_argument(
        "same_component error: graph must not be nullptr!");
  if (id1 < 0 || id1 >= GraphType::NUM || !graph->vertexes.at(id1) || id2 < 0 ||
      id2 >= GraphType::NUM || !graph->vertexes.at(id2))
    throw std::invalid_argument("same_component error: id muse belongs [0,N) "
                                "and graph->vertexes[id] must not be nullptr!");

  if (NodeType::find_set(graph->vertexes.at(id1)->node) ==
      NodeType::find_set(graph->vertexes.at(id2)->node)) {
    return true;
  } else
    return false;
}
