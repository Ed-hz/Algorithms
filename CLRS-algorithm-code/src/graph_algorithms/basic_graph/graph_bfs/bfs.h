 ...
 * \param
 * pre_action:一个可调用对象，在每次发现一个顶点时调用，调用参数为该顶点的`id`。默认为空操作，即不进行任何操作
 * \param
 * post_action:一个可调用对象，在每次对一个顶点搜索完成时调用，调用参数为该顶点的`id`。默认为空操作，即不进行任何操作
 * \return:void
 *
 * `source_id`在以下情况下无效：
 *
 ...
 *
 * 给定图
 * G=(V,E)和一个可以识别的源点`s`。。所有的结点在一开始都被涂上白色。每个结点的颜色存放在属性`color`中；
 ...
 * 算法的时间复杂度为 O(E+V)
 *
 * 最短路径：广度优先搜索能找出给定源结点s到所有可以到达的结点之间的距离。定义从源s到结点v之间的最短路径距离
 * delt(s,v) 为从结点s到v之间的所有路径里面最少的边数。 如果从s到v没有路径，则
 * delt(s,v)=正无穷大 。我们定义从s到v之间的长度为 delt(s,v) 的路径为 s 到 v
 * 的最短路径。可以证明：广度优先搜索可以正确计算出最短路径距离。
 *
 * 广度优先树：对于G=(V,E)和源点s，定义图G的前驱子图为 G_pai=(V_pai,E_pai)，其中
 * V_pai={ v属于V:
 * v.parent!=NIL}并上{s}，E_pai={(v.parent,v):v属于(V_pai-{s})}。
...
void breadth_first_search(
    ...
    std::function<void(typename GraphType::VIDType)> pre_action =
        [](typename GraphType::VIDType) {},
    std::function<void(typename GraphType::VIDType)> post_action =
        [](typename GraphType::VIDType) {}) {
  ...
  if (!graph)
    throw std::invalid_argument(
        "breadth_first_search error: graph must not be nullptr!");
  if (source_id < 0 || source_id >= GraphType::NUM ||
      !graph->vertexes.at(source_id))
    throw std::invalid_argument(
        "breadth_first_search error: source_id muse belongs [0,N) and "
