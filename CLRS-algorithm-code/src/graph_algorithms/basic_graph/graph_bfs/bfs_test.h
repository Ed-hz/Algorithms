...
public:
  typedef Graph<BFS_N, BFS_Vertex<int>>
      GType; /*!< 模板实例化的图类型，该图的顶点类型为`BFS_Vertex<int>`*/
  typedef std::function<void(BFS_Vertex<int>::VIDType v_id)>
...
TEST_F(BFSTest, test_bfs) {
  {
    ...
    ActionType print_bfs = [&os](BFS_Vertex<int>::VIDType v_id) {
      os << v_id << ",";
    };
    //****** 测试只有一个顶点的图**********
    EXPECT_THROW(breadth_first_search(_1v_graph, 1, print_bfs),
  ...
  {
    std::ostringstream os;
    ActionType print_bfs = [&os](BFS_Vertex<double>::VIDType v_id) {
      os << v_id << ",";
    };
    //***** 测试只有一条边的图*********
...
TEST_F(BFSTest, test_get_path) {
  ...
  //****** 测试只有一个顶点的图**********
  EXPECT_EQ(
      get_path(_1v_graph->vertexes.at(0), _1v_graph->vertexes.at(0)).size(), 1);
  EXPECT_THROW(get_path(_1v_graph->vertexes.at(0), _1v_graph->vertexes.at(1)),
               std::invalid_argument);
  //***** 测试只有一条边的图*********
  _1e_graph->vertexes.at(1)->set_found(_1e_graph->vertexes.at(0));
  EXPECT_EQ(get_path(_1e_graph->vertexes.at(0), _1e_graph->vertexes.at(1)),
            std::vector<int>({0, 1}));
  ...
  for (int i = 0; i < BFS_N; i++) {
    ...
    EXPECT_EQ(get_path(_list_graph->vertexes.at(i),
                       _list_graph->vertexes.at(BFS_N - 1)),
              real)
        << "i:" << i;
  }
}
