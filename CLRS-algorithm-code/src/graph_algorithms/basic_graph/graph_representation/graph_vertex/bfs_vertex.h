...
template <typename KType> struct BFS_Vertex : public Vertex<KType> {
   ...
   * 顶点的颜色默认设为白色
   */
  explicit BFS_Vertex(const KeyType &k)
      : color(COLOR::WHITE), Vertex<KeyType>(k) {}
  //!显式构造函数，指定`key`和编号
  /*!
   * \param  k:顶点存放的数据
   * \param  d:顶点的编号
  ...
  BFS_Vertex(const KeyType &k, VIDType d)
      : color(COLOR::WHITE), Vertex<KeyType>(k, d) {}
  ...
  void set_found(std::shared_ptr<BFS_Vertex> v_parent) {
    if (!v_parent)
      throw std::invalid_argument(
          "set_found error: v_parent must not be null!");
    color = COLOR::GRAY;
    parent = v_parent;
    if (v_parent->key == unlimit<KeyType>())
      this->key = v_parent->key;
    else
