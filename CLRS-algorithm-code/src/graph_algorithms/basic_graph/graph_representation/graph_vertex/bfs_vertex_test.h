...
/*!
 *
 * `BFSVertexTest`是 `::testing::Test`
 * 的子类。它主要用于为每一个`TEST_F`准备测试环境
 */
class BFSVertexTest : public ::testing::Test {
public:
  typedef BFS_Vertex<double>
      BFS_Vertex; /*!< 模板实例化的顶点类型为`BFS_Vertex<double>`*/
...
TEST_F(BFSVertexTest, test_set_found) {
  EXPECT_THROW(_normal_vertex->set_found(std::shared_ptr<BFS_Vertex>()),
...
TEST_F(BFSVertexTest, test_to_string) {
  EXPECT_EQ(_normal_vertex->to_string(),
            "vertex id:-1\t key:1\t color:WHITE\t parent-id:nullptr");
  _normal_vertex->set_found(_default_vertex);
  EXPECT_EQ(_normal_vertex->to_string(),
            "vertex id:-1\t key:1\t color:GRAY\t parent-id:-1");
}
