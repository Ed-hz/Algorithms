...
/*!
 *
 * `DFSVertexTest`是 `::testing::Test`
 * 的子类。它主要用于为每一个`TEST_F`准备测试环境
 */
class DFSVertexTest : public ::testing::Test {
public:
  typedef DFS_Vertex<double>
      DFS_Vertex; /*!< 模板实例化的顶点类型为`DFS_Vertex<double>`*/
...
TEST_F(DFSVertexTest, test_to_string) {
  EXPECT_EQ(_normal_vertex->to_string(),
            ...
            "discover_time:-1\t finish_time:-1");
  _normal_vertex->parent = _default_vertex;
  EXPECT_EQ(_normal_vertex->to_string(),
            "vertex id:-1\t key:1\t color:WHITE\t parent-id:-1\t "
            "discover_time:-1\t finish_time:-1");
}
