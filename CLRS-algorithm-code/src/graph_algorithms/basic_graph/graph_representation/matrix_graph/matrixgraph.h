 ...
 * 图的矩阵主要包含两个数据：
 *
 * - `matrix`：一个矩阵，类型为`std::array<std::array<EWeightType,N>, N>
 * `，为`N*N`大小：
 * -
...
template <unsigned N> struct MatrixGraph {
  ...
  typedef std::tuple<VIDType, VIDType, EWeightType>
      EdgeTupleType; /*!< 边的三元素（顶点1编号，顶点2编号，权重)组成的元组*/
  ...
  bool has_edge(VIDType id_from, VIDType id_to) const {
    if (id_from < 0 || id_from >= N || id_to < 0 || id_to >= N) {
      throw std::invalid_argument(
          "has_edge: id_from  and id _to must belongs [0,N),");
    }
