...
/*!
 *
 * `GraphTest`是 `::testing::Test`
 * 的子类。它主要用于为每一个`TEST_F`准备测试环境
 */
class GraphTest : public ::testing::Test {
public:
  typedef Graph<G_N, Vertex<double>>
      GType; /*!< 模板实例化的图类型，该图的顶点类型为`Vertex<double>`*/
...
protected:
  ...
  std::shared_ptr<GType>
      _n_vertexes_m_edges_graph; /*!< 指向一个含有n结点,m条边的图*/
};
...
TEST_F(GraphTest, test_add_vertex) {
  ...
  for (int i = 0; i < G_N; i++) //满图：不可以添加顶点
  {
    ...
    EXPECT_THROW(_n_vertexes_m_edges_graph->add_vertex(i),
...
TEST_F(GraphTest, test_add_vertex_with_id) {
  ...
  for (int i = 0; i < G_N; i++) //满图：不可以添加顶点
  {
    ...
    EXPECT_THROW(_n_vertexes_m_edges_graph->add_vertex(i, i),
                 std::invalid_argument);
  }
}
//! graph_test:Graph
...
TEST_F(GraphTest, test_modify_vertex) {
  ...
  for (int i = 0; i < G_N; i++) //满图：可以任意修改顶点
  {
    ...
    EXPECT_NEAR(_n_vertexes_m_edges_graph->vertexes.at(i)->key, 1 + i / G_N,
                0.01);
...
TEST_F(GraphTest, test_weight) {
  for (int i = 0; i < G_N; i++)
    for (int j = 0; j < G_N; j++) {
      EXPECT_THROW(_empty_graph->weight(i, j),
                   std::invalid_argument); //空图：没有顶点没有边
      EXPECT_THROW(_n_vertexes_graph->weight(i, j),
                   std::invalid_argument); //无边有顶点的图：没有边
      if (j <= i)
        EXPECT_THROW(_n_vertexes_m_edges_graph->weight(i, j),
                     std::invalid_argument);
      else
...
TEST_F(GraphTest, test_has_edge) {
  for (int i = 0; i < G_N; i++)
    for (int j = 0; j < G_N; j++) {
      EXPECT_THROW(_empty_graph->has_edge(i, j),
                   std::invalid_argument); //空图：没有顶点没有边
...
TEST_F(GraphTest, test_add_edge) {
  for (int i = 0; i < G_N; i++)
    for (int j = 0; j < G_N; j++) {
      EXPECT_THROW(_empty_graph->add_edge(std::make_tuple(i, j, i * j)),
                   std::invalid_argument); //空图：没有顶点没有边
      _n_vertexes_graph->add_edge(
          std::make_tuple(i, j, i * j)); //无边有顶点的图：没有边
...
TEST_F(GraphTest, test_add_edges) {
  ...
  EXPECT_THROW(_empty_graph->add_edges(tuples.begin(), tuples.end()),
               std::invalid_argument); //空图：没有顶点没有边
  _n_vertexes_graph->add_edges(tuples.begin(),
                               tuples.end()); //无边有顶点的图：没有边
  EXPECT_EQ(_n_vertexes_graph->edge_tuples(), tuples);
  EXPECT_THROW(
      _n_vertexes_m_edges_graph->add_edges(tuples.begin(), tuples.end()),
      std::invalid_argument);
}
//! graph_test:Graph
/*!
...
TEST_F(GraphTest, test_adjust_edge) {
  for (int i = 0; i < G_N; i++)
    for (int j = 0; j < G_N; j++) {
      EXPECT_THROW(_empty_graph->adjust_edge(i, j, 0),
                   std::invalid_argument); //空图：没有顶点没有边
      EXPECT_THROW(_n_vertexes_graph->adjust_edge(i, j, 0),
                   std::invalid_argument); //无边有顶点的图：没有边
      if (j <= i) {
        EXPECT_THROW(_n_vertexes_m_edges_graph->adjust_edge(i, j, 0),
                     std::invalid_argument);
      } else {
...
TEST_F(GraphTest, test_edge_tuples) {
  ...
  EXPECT_EQ(_n_vertexes_graph->edge_tuples().size(),
            0); //无边有顶点的图：没有边
...
TEST_F(GraphTest, test_vertex_edge_tuples) {
  for (int i = 0; i < G_N; i++) {
    EXPECT_THROW(_empty_graph->vertex_edge_tuples(i),
                 std::invalid_argument); //空图：没有顶点没有边
    EXPECT_EQ(_n_vertexes_graph->vertex_edge_tuples(i).size(),
              0); //无边有顶点的图：没有边
  }
...
TEST_F(GraphTest, test_inverse) {
  ...
  //无效边权重相同
  EXPECT_EQ(empty_inverse->matrix.invalid_weight,
            _empty_graph->matrix.invalid_weight);
  EXPECT_EQ(_n_vertexes_inverse->matrix.invalid_weight,
            _n_vertexes_graph->matrix.invalid_weight);
  EXPECT_EQ(_n_vertexes_m_edges_inverse->matrix.invalid_weight,
            _n_vertexes_m_edges_graph->matrix.invalid_weight);

  //顶点相同
  for (int i = 0; i < G_N; i++) {
    if (empty_inverse->vertexes.at(i)) {
      EXPECT_EQ(empty_inverse->vertexes.at(i)->id,
                _empty_graph->vertexes.at(i)->id);
      EXPECT_EQ(empty_inverse->vertexes.at(i)->key,
                _empty_graph->vertexes.at(i)->key);
    } else
      EXPECT_FALSE(_empty_graph->vertexes.at(i));

    if (_n_vertexes_inverse->vertexes.at(i)) {
      EXPECT_EQ(_n_vertexes_inverse->vertexes.at(i)->id,
                _n_vertexes_graph->vertexes.at(i)->id);
      EXPECT_EQ(_n_vertexes_inverse->vertexes.at(i)->key,
                _n_vertexes_graph->vertexes.at(i)->key);
    } else
      EXPECT_FALSE(_n_vertexes_graph->vertexes.at(i));

    if (_n_vertexes_m_edges_inverse->vertexes.at(i)) {
      EXPECT_EQ(_n_vertexes_m_edges_inverse->vertexes.at(i)->id,
                _n_vertexes_m_edges_graph->vertexes.at(i)->id);
      EXPECT_EQ(_n_vertexes_m_edges_inverse->vertexes.at(i)->key,
                _n_vertexes_m_edges_graph->vertexes.at(i)->key);
    } else
      EXPECT_FALSE(_n_vertexes_m_edges_graph->vertexes.at(i));
  }
  //边反向
  EXPECT_EQ(empty_inverse->edge_tuples().size(), 0);
  EXPECT_EQ(_n_vertexes_inverse->edge_tuples().size(), 0);
  EXPECT_EQ(_n_vertexes_m_edges_inverse->edge_tuples().size(),
            _n_vertexes_m_edges_graph->edge_tuples().size());
  ...
  for (int i = 0; i < edges.size(); i++) {
    ...
    auto r_edge = std::make_tuple(std::get<1>(edge), std::get<0>(edge),
                                  std::get<2>(edge));
    bool founded = false;
