...
public:
  typedef Graph<DFS_N, DFS_Vertex<double>>
      GType; /*!< 模板实例化的图类型，该图的顶点类型为`DFS_Vertex<double>`*/
  typedef std::function<void(DFS_Vertex<double>::VIDType v_id, int time)>
...
protected:
  void SetUp() {
    ...
    for (int i = 0; i < DFS_N - 1; i++)
      _list_graph->add_edge(
          std::make_tuple(i, i + 1, 10 + i)); //该图的边是从左到右组成一个链条
  ...
  std::shared_ptr<GType>
      _list_graph; /*!< 指向一个图，该图的边组成一个链条，该图用顺序搜索*/
  std::shared_ptr<GType>
      _rlist_graph; /*!< 指向一个图，该图的边组成一个链条，该图用逆序搜索*/
};
...
TEST_F(DFSTest, test_dfs) {
  {
    ...
    ActionType print_discover = [&discover_os](DFS_Vertex<double>::VIDType v_id,
                                               int time) {
      discover_os << v_id << ",";
    };
    ActionType print_finished = [&finished_os](DFS_Vertex<double>::VIDType v_id,
                                               int time) {
      finished_os << v_id << ",";
    };
    //****** 测试只有一个顶点的图**********
  ...
  }
  {
    ...
    depth_first_search(_rlist_graph, print_discover, print_finished,
                       empty_action, empty_action, search_order);
    EXPECT_EQ(discover_os.str(), real_discover_str);
    EXPECT_EQ(finished_os.str(), real_finish_str);
  }
}
...
TEST_F(DFSTest, test_get_path) {
  ...
  for (int i = 0; i < DFS_N; i++) {
    ...
    for (int j = i; j < DFS_N; j++)
      real.push_back(j);
    EXPECT_EQ(get_path(_list_graph->vertexes.at(i),
                       _list_graph->vertexes.at(DFS_N - 1)),
              real)
        << "i:" << i;
  }
}
