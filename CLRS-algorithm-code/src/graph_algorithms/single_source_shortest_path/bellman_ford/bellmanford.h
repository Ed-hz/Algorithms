...
void initialize_single_source(std::shared_ptr<GraphType> graph,
                              typename GraphType::VIDType source_id) {
  if (!graph)
    throw std::invalid_argument(
        "initialize_single_source error: graph must not be nullptr!");
  if (source_id < 0 || source_id >= GraphType::NUM ||
      !graph->vertexes.at(source_id))
    throw std::invalid_argument(
        "initialize_single_source error: source_id muse belongs [0,N) and "
        "graph->vertexes[source_id] must not be nullptr!");
 ...
 * \param
 * to：松弛有向边的终止结点，必须非空且不等于from。若为空或者等于from则抛出异常
 * \param weight:有向边的权重
 * \return: void
 *
 *
...
void relax(std::shared_ptr<VertexType> from, std::shared_ptr<VertexType> to,
           typename VertexType::KeyType weight) {
  if (!from || !to)
    throw std::invalid_argument(
        "relax error: from_vertex and to_vertex must not be nullptr!");
  if (from == to)
    throw std::invalid_argument(
        "relax error: from_vertex must not be to_vertex!");
  if ((from->key + weight) >=
      unlimit<
          typename VertexType::KeyType>()) // u.key+weight为正无穷，则不可能松弛
    return;
 ...
 * w(p)=w(v0,v1)+w(v1,v2)+...+w(v(k-1),vk)。定义结点u到结点v的最短路径权重
 * delt(u,v)为：
 *
 ...
 *
 * 给定图G=(V,E)，对每个结点v我们维持一个前驱结点v.pai。在最短路径算法中，由pai值诱导的前驱子图G_pai=(V_pai,E_pai)，其中
 * V_pai={v属于V:v.pai!=nil}并上源点s，
 * E_pai是V_pai中所有结点的pai值诱导的边的集合：E_pai={(v.pai,v)属于E:v属于V_pai-{s}
 * }。算法终止时，G_pai是一棵最短路径树：该树包含了从源结点s
 * 到每个可以从s到达的结点的一条最短路径。
 *
 ...
 */
template <typename GraphType>
bool bellman_ford(std::shared_ptr<GraphType> graph,
                  typename GraphType::VIDType source_id) {

  if (!graph)
    throw std::invalid_argument(
        "bellman_ford error: graph must not be nullptr!");
  if (source_id < 0 || source_id >= GraphType::NUM ||
      !graph->vertexes.at(source_id))
    throw std::invalid_argument(
        "bellman_ford error: source_id muse belongs [0,N) and "
        "graph->vertexes[source_id] must not be nullptr!");
  ...
  for (int i = 0; i < GraphType::NUM - 1; i++) {
    for (const auto &edge : graph->edge_tuples()) {
      relax(graph->vertexes[std::get<0>(edge)],
            graph->vertexes[std::get<1>(edge)], std::get<2>(edge));
    }
  }
  //**********  检验是否存在从源点可达的权重为负的环路 *************
  for (const auto &edge : graph->edge_tuples()) {
    if (graph->vertexes[std::get<1>(edge)]->key >
        graph->vertexes[std::get<0>(edge)]->key + std::get<2>(edge))
      return false;
  }
  return true;
}
