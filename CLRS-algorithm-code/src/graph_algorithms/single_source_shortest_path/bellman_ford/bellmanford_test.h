...
/*!
 *
 * `BellmanFordTest`是 `::testing::Test`
 * 的子类。它主要用于为每一个`TEST_F`准备测试环境
 */
class BellmanFordTest : public ::testing::Test {
public:
  typedef Graph<B_NUM, VertexP<int>>
      GraphType; /*!< 模板实例化的图类型，该图的顶点类型为`VertexP<int>`*/
...
protected:
  void SetUp() {
    ...
    for (int i = 0; i < B_NUM - 1; i++)
      _normal_graph->add_edge(
          std::make_tuple(i, i + 1, 1)); //该图的边是从左到右组成一个链条

    //****  含顶点图和边图：10个顶点，10条边 :0-->1-->2....-->9(权重均为1)以及
    //9-->8(权重-2)  ****
  ...
  std::shared_ptr<GraphType>
      _normal_graph; /*!< 指向一个图，该图的边组成一条链条*/
  std::shared_ptr<GraphType>
      _minus_graph; /*!< 指向一个图，该图的边包含负权值回路*/
};
...
TEST_F(BellmanFordTest, test_initialize_single_source) {
  EXPECT_THROW(initialize_single_source(std::shared_ptr<GraphType>(), 0),
  ...
  for (int i = 0; i < B_NUM; i++) {
    ...
    if (i == 0)
      EXPECT_EQ(_normal_graph->vertexes[i]->key, 0);
    else
      EXPECT_EQ(_normal_graph->vertexes[i]->key,
                unlimit<VertexType::KeyType>());
  }
}
...
TEST_F(BellmanFordTest, test_relax) {
  ...
  relax(_normal_graph->vertexes[0], _normal_graph->vertexes[1],
        _normal_graph->weight(0, 1));
...
TEST_F(BellmanFordTest, test_bellman_ford) {
  ...
  for (int i = 1; i < B_NUM; i++) {
    ...
    EXPECT_EQ(_normal_graph->vertexes[i]->parent,
              _normal_graph->vertexes[i - 1]);
  }
  //*********** 含有负权值回路的图  ***********
