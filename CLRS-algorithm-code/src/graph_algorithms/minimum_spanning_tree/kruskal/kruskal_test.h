...
/*!
 *
 * `KruskalTest`是 `::testing::Test`
 * 的子类。它主要用于为每一个`TEST_F`准备测试环境
 */
class KruskalTest : public ::testing::Test {
public:
  typedef Graph<K_NUM, SetVertex<int>>
      GType; /*!< 模板实例化的图类型，该图的顶点类型为`SetVertex<int>`*/
  typedef std::function<void(SetVertex<int>::VIDType, SetVertex<int>::VIDType)>
      ActionType; /*!< 模板实例化的Action类型*/
protected:
  void SetUp() {
    _1v_graph = std::make_shared<GType>(-1); //边的无效权重为-1
    _1v_graph->add_vertex(0);                //该图只有一个顶点
    _1v_graph->add_edge(
        std::make_tuple(0, 0, 1)); //该图只有一个顶点，必须有一条边指向自己
    ...
    for (int i = 0; i < K_NUM - 1; i++)
      _list_graph->add_edge(
          std::make_tuple(i, i + 1, i + 1)); //该图的边是从左到右组成一个链条
    ...
    for (int i = 0; i < K_NUM; i++)
      for (int j = 0; j < K_NUM; j++)
        if (i == j)
          ...
          _all_edges_graph->add_edge(
              std::make_tuple(i, j, i + j)); //该图中任意一对顶点之间都有边
  }
  void TearDown() {}
  ...
  std::shared_ptr<GType>
      _all_edges_graph; /*!< 指向一个图，该图中任意一对顶点之间都有边*/
};
...
TEST_F(KruskalTest, test_kruskal) {
  ...
  //************* 单点图 **************
  {
    ...
    ActionType pre_action = [&edges](GType::VIDType id1, GType::VIDType id2) {
  ...
  //************** 单边图 ******************
  {
    ...
    EXPECT_EQ(node0->parent,
              node1); // tree_root:u, tree_root:v,
                      // rank小的挂在rank大的之下；若二者rank相等，则u挂在v下
  ...
  //************** 单链图 ******************
  {
    ...
    for (int i = 0; i < K_NUM;
         i++) // tree_root:u, tree_root:v,
              // rank小的挂在rank大的之下；若二者rank相等，则u挂在v下
      EXPECT_EQ(_list_graph->vertexes.at(i)->node->parent,
                _list_graph->vertexes.at(1)->node); //所以结点1是最终根结点

    //检验添加边的数量和顺序
    EXPECT_EQ(edges.size(), result_edges.size());
    for (int i = 0; i < edges.size(); i++)
      EXPECT_TRUE((edges[i].first == result_edges[i].first &&
                   edges[i].second == result_edges[i].second) ||
                  (edges[i].first == result_edges[i].second &&
                   edges[i].second == result_edges[i].first));
  }
  //***************** 任意一对顶点之间都有边的图  **************
  {
    ...
    for (int i = 0; i < K_NUM;
         i++) // tree_root:u, tree_root:v,
              // rank小的挂在rank大的之下；若二者rank相等，则u挂在v下
      EXPECT_EQ(_all_edges_graph->vertexes.at(i)->node->parent,
                _all_edges_graph->vertexes.at(1)->node); //所以结点1是最终根结点
