 ...
 * \param
 * pre_action:一个可调用对象，在每次从最小优先级队列中弹出最小顶点时立即调用，调用参数为该顶点的`id`。默认为空操作，即不进行任何操作
 * \param
 * post_action:一个可调用对象，在每次从最小优先级队列中弹出最小顶点并处理完它的边时立即调用，调用参数为该顶点的`id`。默认为空操作，即不进行任何操作
 * \return: 最小生成树的权重
 *
 * `source_id`在以下情况下无效：
 *
 ...
 */
template <typename GraphType, typename ActionType = std::function<
                                  void(typename GraphType::VIDType)>>
typename GraphType::EWeightType prim(
    std::shared_ptr<GraphType> graph, typename GraphType::VIDType source_id,
    ActionType pre_action = [](typename GraphType::VIDType) {},
    ActionType post_action = [](typename GraphType::VIDType) {}) {
  ...
  if (source_id < 0 || source_id >= GraphType::NUM ||
    ...
    throw std::invalid_argument(
        "prim error: source_id muse belongs [0,N) and "
        "graph->vertexes[source_id] must not be nullptr!");

  auto vertex_compare = [](std::shared_ptr<VertexType> first,
                           std::shared_ptr<VertexType> second) {
    return first->key < second->key;
  };
  auto vertex_key_get = [](std::shared_ptr<VertexType> v) -> KeyType & {
    return v->key;
  };
  ...
  while (!min_queue.is_empty()) {
    ...
    for (const auto &edge_tuple : edges) {
      ...
      if (index >= 0 && edge_weight < another_vertex->key) {
        ...
        min_queue.decreate_key(
            index,
            edge_weight); //这里隐含着一个最小优先级队列的decreate_key操作
      }
    }
