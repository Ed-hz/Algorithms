...
public:
  typedef Graph<PRIM_N, VertexP<int>>
      GType; /*!< 模板实例化的图类型，该图的顶点类型为`VertexP<int>`*/
  typedef std::function<void(VertexP<int>::VIDType v_id)>
...
TEST_F(PrimTest, test_prim) {
  ...
  //************** 单链图 ******************
  {
    ...
    for (int i = 0; i < PRIM_N; i++) {
      ...
      if (i == 0)
        continue;
      EXPECT_EQ(_list_graph->vertexes.at(i)->parent,
                _list_graph->vertexes.at(i - 1));
    }
  ...
  //***************** 任意一对顶点之间都有边的图  **************
  {
    ...
    for (int i = 0; i < PRIM_N; i++) {
      ...
      if (i == 0)
        continue;
      EXPECT_EQ(_all_edges_graph->vertexes.at(i)->parent,
                _all_edges_graph->vertexes.at(0))
          << "i:" << i;
    }
    EXPECT_EQ(ids, result_ids);
  }
