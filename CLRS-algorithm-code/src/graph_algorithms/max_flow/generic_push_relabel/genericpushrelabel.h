 ...
 *
 * 如果一个结点
 * u是一个溢出结点，则至少有一条入边，假设为(v,u)，且f(v,u)>0。此时残留网络G_f中的边c_f(u,v)>0。如果此时还有h(u)=h(v)+1，
 * 则可以对边(u,v)执行push操作。
 *
...
void push(
    std::shared_ptr<GraphType> graph, typename GraphType::VIDType u_id,
    typename GraphType::VIDType v_id,
    std::array<std::array<typename GraphType::EWeightType, GraphType::NUM>,
               GraphType::NUM> &flow) {
  ...
  if (graph->has_edge(u_id, v_id)) //(u,v)属于E
    c_f =
        graph->weight(u_id, v_id) - flow[u_id][v_id]; // c_f(u,v)=c(u,v)-f(u,v)
  else if (graph->has_edge(v_id, u_id))               //(v,u)属于E
    c_f = flow[v_id][u_id];                           // c_f(u,v)=f(v,u)
  else
    throw std::invalid_argument(
        "push error: must be (u,v) in E or (v,u) in E!");

  if (c_f == 0)
    throw std::invalid_argument("push error:must be c_f > 0!");
 ...
 */
template <typename GraphType>
typename GraphType::VIDType
min_v_at_Ef(std::shared_ptr<GraphType> graph, typename GraphType::VIDType u_id,
            const std::array<
                std::array<typename GraphType::EWeightType, GraphType::NUM>,
                GraphType::NUM> &flow) {
  typedef typename GraphType::EWeightType EWeightType;
  typedef typename GraphType::VIDType VIDType;
  typedef std::array<std::array<EWeightType, GraphType::NUM>, GraphType::NUM>
      MatrixType;

  if (!graph)
    throw std::invalid_argument(
        "min_v_at_Ef error: graph must not be nullptr!");
  if (u_id < 0 || u_id >= GraphType::NUM)
    throw std::invalid_argument("min_v_at_Ef error:id must >=0 and <N.");
  if (!graph->vertexes.at(u_id))
    throw std::invalid_argument(
        "min_v_at_Ef error: vertex of id does not exist.");
 ...
 */
template <typename GraphType>
void relabel(std::shared_ptr<GraphType> graph, typename GraphType::VIDType u_id,
             const std::array<
                 std::array<typename GraphType::EWeightType, GraphType::NUM>,
                 GraphType::NUM> &flow) {
  ...
  if (min_v_id < 0)
    throw std::invalid_argument(
        "relabel error: there must be edges in E_f start from u !");
 ...
 *   - 初始化超额流量 e： e(u)=c(s,u)，u为与源s相邻的结点;
 * e(u)=0，u为与源s不相邻的结点; e(s)初始化为所有s出发的管道之后的相反数 >
 * 由顶点的`key`属性存储超额流量e
 */
template <typename GraphType>
void initialize_preflow(
    std::shared_ptr<GraphType> graph, typename GraphType::VIDType src,
    std::array<std::array<typename GraphType::EWeightType, GraphType::NUM>,
               GraphType::NUM> &flow) {
  if (!graph)
    throw std::invalid_argument(
        "initialize_preflow error: graph must not be nullptr!");
  if (src < 0 || src >= GraphType::NUM)
    throw std::invalid_argument("initialize_preflow error:id must >=0 and <N.");
  if (!graph->vertexes.at(src))
    throw std::invalid_argument(
        "initialize_preflow error: vertex of id does not exist.");
 ...
 * ### 算法原理
 *
 * 目前最大流的最快实现是基于推送-重贴标签算法。推送-重贴标签算法比
 * Ford-fulkserson的局域性更强。它不是对整个残余网络进行检查，
 * 而是一个结点一个结点的查看，每一步只检查当前结点的邻结点。与 Ford-fulkerson
 * 方法不同，
 * 推送-重贴标签算法并不在整个执行过程中保持流量守恒性质，而是维持一个预流，该流是一个
 * V*V --> R的函数f,它满足容量限制单不满足流量守恒性质。
 * 进入一个点的流量可以超过流出该结点的流量，我们称结点u的
 * （进入流量-流出流量）=e(u)为结点u的超额流。 对于结点u属于V-{s,t}，如果
 * e(u)>0，则称结点u溢出。
 *
 ...
 *
 * 高度h满足下面性质：h(s)=|V|,h(t)=0,对于所有的边(u,v)属于E_f（残余网络中的边），则有
 * h(u)<=h(v)+1
 *
 ...
 *   - 如果不能执行 push 操作，又由于存在超额流量 e>0 的结点，因此必然可以执行
 * relabel 操作。则执行 relabel 操作
 *
 *
 * ### 算法性能
 *
...
std::array<std::array<typename GraphType::EWeightType, GraphType::NUM>,
           GraphType::NUM>
generic_push_relabel(std::shared_ptr<GraphType> graph,
                     typename GraphType::VIDType src,
                     typename GraphType::VIDType dst) {
  typedef typename GraphType::EWeightType EWeightType;
  typedef std::array<std::array<EWeightType, GraphType::NUM>, GraphType::NUM>
      MatrixType;
  if (!graph)
    throw std::invalid_argument(
        "generic_push_relabel error: graph must not be nullptr!");
  if (src < 0 || src >= GraphType::NUM || dst < 0 || dst >= GraphType::NUM)
    throw std::invalid_argument(
        "generic_push_relabel error:id must >=0 and <N.");
  if (!graph->vertexes.at(src) || !graph->vertexes.at(dst))
    throw std::invalid_argument(
        "generic_push_relabel error: vertex of id does not exist.");
  MatrixType flow;
