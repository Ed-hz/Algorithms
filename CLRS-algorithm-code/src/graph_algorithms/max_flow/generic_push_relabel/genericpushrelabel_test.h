...
/*!
 *
 * `GenericPushRelabelTest`是 `::testing::Test`
 * 的子类。它主要用于为每一个`TEST_F`准备测试环境
 */
class GenericPushRelabelTest : public ::testing::Test {
public:
  typedef Graph<PR_N, FlowVertex<int>>
      GType; /*!< 模板实例化的图类型，该图的顶点类型为`FlowVertex<int>`*/
...
TEST_F(GenericPushRelabelTest, test_initialize_preflow) {
  ...
  for (int i = 0; i < PR_N; i++)
    for (int j = 0; j < PR_N; j++) {
      if (i == 0 && j == 1)
        EXPECT_EQ(flow[i][j], 16);
      else if (i == 0 && j == 2)
        EXPECT_EQ(flow[i][j], 13);
      else
        EXPECT_EQ(flow[i][j], 0);
    }
}
...
TEST_F(GenericPushRelabelTest, test_generic_push_relabel) {
  typedef std::array<std::array<typename GType::EWeightType, PR_N>, PR_N>
      MatrixType;
  MatrixType real_flow = {
      std::array<typename GType::EWeightType, PR_N>({0, 12, 11, 0, 0, 0}),
