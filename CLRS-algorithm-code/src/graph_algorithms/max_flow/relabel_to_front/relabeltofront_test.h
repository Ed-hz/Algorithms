...
/*!
 *
 * `RelabelToFrontTest`是 `::testing::Test`
 * 的子类。它主要用于为每一个`TEST_F`准备测试环境
 */
class RelabelToFrontTest : public ::testing::Test {
public:
  typedef Graph<RTF_N, FrontFlowVertex<int>>
      GType; /*!< 模板实例化的图类型，该图的顶点类型为`FrontFlowVertex<int>`*/
...
TEST_F(RelabelToFrontTest, test_relabel_to_front) {
  typedef std::array<std::array<typename GType::EWeightType, RTF_N>, RTF_N>
      MatrixType;
  MatrixType real_flow = {
      std::array<typename GType::EWeightType, RTF_N>({0, 12, 11, 0, 0, 0}),
