 ...
 * - 若(u,v)属于E，则存在边(v,u)属于Ef,且
 * cf(v,u)=c(u,v)，表示沿着(u,v)的反方向（即(v,u)方向)可以压入cf大小的反向流量
 * >
...
std::shared_ptr<GraphType> create_Gf(
  ...
  if (graph->matrix.invalid_weight != 0)
    throw std::invalid_argument(
        "create_Gf error: graph invalid weight must be 0!");

  int new_invalid_weight = 0;
  std::shared_ptr<GraphType> new_graph =
      std::make_shared<GraphType>(new_invalid_weight);
  //*************  创建新图的顶点  ******************
  ...
  for (int i = 0; i < GraphType::NUM; i++)
    for (int j = 0; j < GraphType::NUM; j++) {
      if (i == j)
        continue;
      if (!graph->vertexes[i] || !graph->vertexes[j])
        continue;
      if (graph->has_edge(i, j)) //(u,v)属于E
        new_edges.push_back(
            std::make_tuple(i, j, graph->weight(i, j) - flow[i][j]));
 ...
 *
 * ford_fulkerson
 * 算法更准确的称呼是一种方法而不是算法，因为它包含了几种运行时间各不相同的具体实现。
 ...
 * p上能够为每条边增加的流量的最大值为路径p的残余容量，定义为 cf(p)= min
 * {cf(u,v):(u,v)属于路径p}
 *
 * #### 最大流和最小切割定理
 *
 ...
 *       - 取出增广路径的残余容量 cf(p)= min
 * {cf(u,v):(u,v)属于路径p}，然后更新增广路径p中的边(u,v)：
 ...
 *
 * 假定最大流问题中的容量均为整数，由于流量值每次迭代中最少增加一个单位，因此算法运行时间为
 * O(E|f*|) ,|f*|为最大流的值
 *
 *
 */
...
std::array<std::array<typename GraphType::EWeightType, GraphType::NUM>,
           GraphType::NUM>
ford_fulkerson(const std::shared_ptr<GraphType> graph,
  ...
  if (!graph)
    throw std::invalid_argument(
        "ford_fulkerson error: graph must not be nullptr!");
  ...
  if (!graph->vertexes.at(src) || !graph->vertexes.at(dst))
    throw std::invalid_argument(
        "ford_fulkerson error: vertex of id does not exist.");
