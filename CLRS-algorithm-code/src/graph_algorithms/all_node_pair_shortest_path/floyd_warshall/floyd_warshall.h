 ...
 * \return: 一个n*n的矩阵(d_i_j)与n*n的矩阵(p_i_j)的std::pair，其中 d_i_j
 * 代表的是结点i到j的一条最短路径的权重, p_i_j
 * 为从结点i到j的一条最短路径上j的前驱结点
 *
 * ## 所有结点对的最短路径
 *
 ...
 * - 若结点k是路径p上的中间结点，则路径p分解为
 * i-->k(经过p1)-->j(经过p2)，则路径p1是结点i到k的中间结点取自集合{1,2...k-1}
 ...
 *
 * 我们可以在计算矩阵D<k>的同时计算出前驱矩阵II，即计算一个矩阵序列
 * II<0>,II<1>...II<k>。这里定义II<k>=(pai_i_j<k>)，
 ...
 *
 * 对于
 * k>=1，若结点k不是路径p的中间结点，则路径p上的所有中间结点都属于集合{1,2,3,...k-1}，则
 * pai_i_j<k>=pai_i_j<k-1>; 若结点k是路径p的中间结点,考虑路径 i-->k-->j，这里
 * k!=j，则pai_i_j<k>=pai_k_j<k-1>。因此当k>=1时，pai_i_j<k>=:
 *
...
floyd_warshall(std::shared_ptr<GraphType> graph) {
  ...
  if (!graph)
    throw std::invalid_argument(
        "floyd_warshall error: graph must not be nullptr!");
  //**************  初始化 D 和 P ************
  ...
  for (int k = 0; k < GraphType::NUM; k++) {
    ...
    for (int i = 0; i < GraphType::NUM; i++) {
      for (int j = 0; j < GraphType::NUM; j++) {
        ...
        EWeightType sum = 0;
        if (D[i][k] == unlimit<EWeightType>() ||
            D[k][j] == unlimit<EWeightType>() ||
            D[i][k] + D[k][j] >= unlimit<EWeightType>())
