 ...
 * \return: 一个新图
 *
 * 根据图graph生成一个新图new_graph。new_graph的顶点
 * graph的顶点加上一个新顶点s；
...
std::shared_ptr<Graph<GraphType::NUM + 1, typename GraphType::VertexType>>
graph_plus_1v(std::shared_ptr<GraphType> graph) {
  ...
  if (!graph)
    throw std::invalid_argument(
        "graph_plus_1v error: graph must not be nullptr!");

  std::shared_ptr<NewGrapyType> new_graph =
      std::make_shared<NewGrapyType>(graph->matrix.invalid_weight);
  //*************  创建新图的顶点  ******************
  ...
  for (int i = 0; i < GraphType::NUM; i++) // 边 (s,v)
  {
    source_edges.push_back(
        std::make_tuple((VIDType)GraphType::NUM, (VIDType)i, (EWeightType)0));
  }
 ...
 *
 * 对于给定的有向图G=(V,E)，权重函数w：E->R
 * 。我们制作一副新图G'=(V',E')，其中V'=V并上{s}，s是一个新结点，s不属于V。
 ...
 * - 在
 * new_graph上，除了新的顶点s之外的所有顶点v,以v为源顶点执行dijkstra过程。D[i][j]等于
 * new_graph 中以i为源点到j的最短路径的权重的修正值，
 * 修正的方法就是重新赋权的逆过程。
 * - 返回矩阵 D
 *
 * ###算法性能
...
johnson(std::shared_ptr<GraphType> graph) {
  ...
  if (!bellman_ford(new_graph, GraphType::NUM)) //新顶点s的id为 GraphType::NUM
    throw std::invalid_argument(
        "johnson error: graph must not contain minus circle!");
