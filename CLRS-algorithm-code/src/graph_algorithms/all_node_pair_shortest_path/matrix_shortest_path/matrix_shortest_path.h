 ...
 * >这里要求
 * MatrixType是一个n*n的矩阵。如果MatrixType不是一个n*n的矩阵，则编译失败
 *
 * 算法步骤如下：
 *
 * - 外层循环 i 从 0...N-1(N次)
 *   - 内层循环 j 从 0...N-1(N次)
 *       - 将newL[i][j]设为正无穷，对所有的k,k 从 0...N-1(N次)，选取
 * L[i][k]+W[k][j]的最小值赋值给newL[i][j]
 * - 最终返回 newL
 *
 * 性能：时间复杂度 O(n^3)
 *
...
MatrixType extend_path(const MatrixType &L, const MatrixType &W) {
  ...
  static_assert(std::tuple_size<MatrixType>::value ==
                    std::tuple_size<typename MatrixType::value_type>::value,
                "extend_path error:matrix must be N*N size!"); //必须是方阵
 ...
 * \return: 一个n*n的矩阵(d_i_j)，其中 d_i_j
 * 代表的是结点i到j的一条最短路径的权重
 *
 * ## 所有结点对的最短路径
 *
 ...
 *
 * 本章讨论的所有结点对最短路径的算法的输出也是一个n*n的矩阵D=(d_i_j)，其中
 * d_i_j 代表的是结点i到j的一条最短路径的权重。
 *
 * 有时候为了解决所有结点对最短路径问题，我们不仅要计算出最短路径权重，还需要计算出前驱结点矩阵
 * II=(pai_i_j)，其中 pai_i_j在i=j或者从i到j
 ...
 * 如果 i=j，则p的权重为0且不包含任何边；如果i和j不同，则将路径p分解为
 * i-->k(经过路径p')-->j，其中路径p'最多包含m-1条边。
 *
 * 定义
 * l_i_j<m>为从结点i到j的最多包含m条边的任意路径中的最小权重，则有：l_i_j<m>=
 *
 * - 0：如果i=j
 * - 正无穷:如果 i!=j
 *
 * 对于m>=1，我们有：
 * l_i_j<m>=min(l_i_j<m-1>，min_(1<=k<=n){l_i_k<m-1>+w_k_j})=min_(1<=k<=n){l_i_k<m-1>+w_k_j}。
 * 如果图G不包含负值的环路，则对于每一对结点i,j，如果他们delt(i,j)<正无穷，则从i到j之间存在一条最短路径。由于该路径是简单路径，
 * 则包含的边最多为n-1条。因此delt(i,j)=l_i_j<n-1>=l_i_j<n>=...
 *
 * matrix_shortest_path算法根据输入矩阵W=(w_i_j)，计算出矩阵序列
 * L<1>，L<2>,...L<n-1>。最后的矩阵L<n-1>包含的是最短路径的权重。
...
matrix_shortest_path(std::shared_ptr<GraphType> graph) {
  ...
  if (!graph)
    throw std::invalid_argument(
        "matrix_shortest_path error: graph must not be nullptr!");
  MatrixType W;
  //**************  从图中创建权重矩阵  ***************
  //****  这里不能直接从图的矩阵描述中提取，因为这里要求
  //w(i,i)=0，而图的矩阵描述中，结点可能有指向自己的边
 ...
 * 由matrix_shortest_path过程定义的矩阵乘法是相关的，因此可以仅用t个矩阵乘积来计算矩阵L<n-1>，其中
 * n为大于lg(n-1)的最小整数。 因此可以用复平方技术来计算上述矩阵序列：
 *
 * - L<1>=W
 * - L<2>=W^2=W.W
 * - L<4>=W^4=W^2.W^2
...
matrix_shortest_path_fast(std::shared_ptr<GraphType> graph) {
  ...
  if (!graph)
    throw std::invalid_argument(
        "matrix_shortest_path_fast error: graph must not be nullptr!");
